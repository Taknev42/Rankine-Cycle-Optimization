...
while P_boiler_initial > 12e6
    eta_variation1(i) = variation(P_boiler_initial,P_condenser_initial);
...
while P_condenser_initial > 5e3
    eta_variation2(j) = variation(P_boiler_initial,P_condenser_initial);
...
plot(pb_array,eta_variation1);
xlabel('Boiler Pressure (in Pa)');
ylabel('Efficiency of the cycle');
title('Efficiency V/S Boiler Pressure');

figure();

plot(pc_array,eta_variation2);
xlabel('Condenser Pressure (in Pa)');
ylabel('Efficiency of the cycle');
title('Efficiency V/S Condenser Pressure');



function eff = variation(p_b,p_c)                % P_b is the Boiler pressure and P_c is the condenser pressure
